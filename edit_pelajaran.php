...
if(isset($_POST['submit'])){
	$id_pelajaran=htmlentities((trim($_POST['id_pelajaran'])));
	$nama_pelajaran=ucwords(htmlentities($_POST['nama_pelajaran']));
	
	$query=mysqli_query($koneksi,"update setup_pelajaran set nama_pelajaran = '$nama_pelajaran' where id_pelajaran = '$id_pelajaran'");
	
	if($query){
		?><script language="javascript">document.location.href="?page=setup_pelajaran&status=1";</script><?php
	}else{
		?><script language="javascript">document.location.href="?page=setup_pelajaran&status=2";</><?php
	}
...
<tr>
    ...
    <div id="content-table-inner">
        ...
        <?php
          ...
          $query=mysqli_query($koneksi, "select * from setup_pelajaran where id_pelajaran = '$id_pelajaran'");
          $row=mysqli_fetch_array($query);
        ?>

        <form action="?page=edit_pelajaran" method="post">
